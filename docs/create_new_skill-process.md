...
### 10. Skill Location
Skills are saved directly to `skills/[skill-name]/` in this repository.

Since this repository is a Claude Code plugin, skills are automatically available as `skill-factory:skill-name` when the plugin is enabled.

Tell user to restart Claude Code to load the new skill.
...
## Output
Save completed skill to `skills/[skill-name]/SKILL.md`.

The skills directory uses a flat structure (no categories). Choose a descriptive, lowercase, hyphenated name that captures the skill's essence.
